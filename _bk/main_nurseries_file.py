...
if __name__ == "__main__":
    ...
    actions = [
        TransformNormalize(
            ...
            normalize=Normalize(normalize_enum=NormalizeActionEnum.ONE_POSITIVE),
        ),
        TransformNormalize(
            column='SOWING_DATE:(date)',
            transform=Transform(transform_enum=TransformEnum.FORCE_ONE),
            normalize=Normalize(normalize_enum=NormalizeActionEnum.PASS),
        ),
        TransformNormalize(
            column='AREA_HARVESTED_BED_PLOT_M2:(m2)',
            transform=Transform(transform_enum=TransformEnum.FILL_AVG),
            normalize=Normalize(normalize_enum=NormalizeActionEnum.ONE_POSITIVE),
        ),
        TransformNormalize(
        ...
        TransformNormalize(
            ...
            normalize=Normalize(normalize_enum=NormalizeActionEnum.ONE_TO_ONE)
        ),
        TransformNormalize(
        ...
        TransformNormalize(
            ...
            transform=Transform(
                ...
                column_coordinate_nswe="GPS Longitude ( E or W):(TEXT)"
            ),
            normalize=Normalize(normalize_enum=NormalizeActionEnum.ONE_POSITIVE)
        ),
        TransformNormalize(
            ...
            transform=Transform(
                transform_enum=TransformEnum.FILL_AVG,
            ),
            normalize=Normalize(normalize_enum=NormalizeActionEnum.ONE_POSITIVE)
        ),
        TransformNormalize(
        ...
        TransformNormalize(
            ...
            transform=Transform(
                transform_enum=TransformEnum.STR_NO,
            ),
            normalize=Normalize(normalize_enum=NormalizeActionEnum.ONE_TO_ONE)
        ),
        TransformNormalize(
    ...
    doRun(
        save_file='test.csv',
        name_file='./app/files/dataset.csv',
        actions=actions,
        remove_rows=remove_rows,
    )
